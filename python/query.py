# -- coding: utf-8 --
## N01
def otc_t_xtrc_oracle(vDiaIni,vDiaFin,vHoraIni,vHoraFin):
    qry="""
(with MOVEMENT_ORDERS AS (
    SELECT object_id,
        NULL AS CONTEXTID,
        'Dekiting' AS type_,
        LOCATION_FROM,
...
SELECT
	...
	nvl(AVG(mol.APPROVED_QUANTITY) - SUM(DECODE(soi.status,9141684386013677572,
...
FROM (
	SELECT *
	FROM R_WORK_ITEM
	WHERE CREATED_WHEN BETWEEN TO_DATE('{vDiaIni} {vHoraIni}', 'DD/MM/YYYY HH24:MI:SS') AND TO_DATE('{vDiaFin} {vHoraFin}', 'DD/MM/YYYY HH24:MI:SS')
) wi
...
WHERE WI.CREATED_WHEN BETWEEN TO_DATE('{vDiaIni} {vHoraIni}', 'DD/MM/YYYY HH24:MI:SS') AND TO_DATE('{vDiaFin} {vHoraFin}', 'DD/MM/YYYY HH24:MI:SS')
